...
let output = input
  ...
  .flat(1)
  .reduce((acum,rec) => (rec===2 || rec===3 || rec===4 || rec===7 ) ?  ++acum : acum, 0  )
...
const left = input
  .map(display => display.split(' | ')[0])
  .map(display => display.split(' ')
    .map(display => ({'sgm':display.split('').sort().join(''), 'len':display.length}))
  ).map( vals => conversor(vals))
  

const right = input
  .map(display => display.split(' | ')[1])
  .map(display => display.split(' ')
    .map(display => display.split('').sort().join('')))
  .map((val, idx) => {
    const res = [];
    for (let item of val) res.push(left[idx].indexOf(item)) 
    return res;
    })
  .map(val => Number(val.reduce((acum, rec) => acum+rec, '')))
  .reduce((acum,rec) => acum+rec, 0);

  
console.log( 'Parte 2', left[0]);   // 495
